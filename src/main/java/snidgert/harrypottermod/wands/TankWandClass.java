...
public class TankWandClass extends Item {
		...
		if (!TankWandClass.Tank) {
			Label_11026: {
				...
				case 9: {
					if (this.getDamage(itemstack) == 9) {
						...
						world.spawnParticle("heart", ((Entity) entityplayer).posX + Item.itemRand.nextFloat() * ((Entity) entityplayer).width * 2.0f - ((Entity) entityplayer).width,
								((Entity) entityplayer).posY, ((Entity) entityplayer).posZ + Item.itemRand.nextFloat() * ((Entity) entityplayer).width * 2.0f - ((Entity) entityplayer).width, d5, d6,
								d7);
						world.spawnParticle("heart", ((Entity) entityplayer).posX + Item.itemRand.nextFloat() * ((Entity) entityplayer).width * 2.0f - ((Entity) entityplayer).width,
								((Entity) entityplayer).posY, ((Entity) entityplayer).posZ + Item.itemRand.nextFloat() * ((Entity) entityplayer).width * 2.0f - ((Entity) entityplayer).width, d5, d6,
								d7);
				...
				case 10: {
					...
					final int k2 = movingobjectposition.blockZ;
					if (!world.isRemote) {
						final List list3 = world.getEntitiesWithinAABB(EntityNagini.class, AxisAlignedBB.getBoundingBox(h, j2, k2, h + 1.0, j2 + 1.0, k2 + 1.0).expand(2.0, 2.0, 2.0));
					...
					if (world.getBlock(h, j2, k2) == Blocks.snow_layer) {
						--j2;
					}
					if (world.isAirBlock(h, j2 + 1, k2)) {
						world.setBlock(h, j2 + 1, k2, Blocks.lava);
					}
					if (world.isAirBlock(h + 1, j2 + 1, k2)) {
						world.setBlock(h + 1, j2 + 1, k2, Blocks.fire);
					}
					if (world.isAirBlock(h - 1, j2 + 1, k2)) {
						world.setBlock(h - 1, j2 + 1, k2, Blocks.fire);
					}
					if (world.isAirBlock(h, j2 + 1, k2 + 1)) {
						world.setBlock(h, j2 + 1, k2 + 1, Blocks.fire);
					}
					if (world.isAirBlock(h, j2 + 1, k2 - 1)) {
						world.setBlock(h, j2 + 1, k2 - 1, Blocks.fire);
					}
				...
				case 11: {
					if (this.getDamage(itemstack) == 11) {
						...
						world.playSoundEffect(((Entity) entityplayer).posX + 0.5, ((Entity) entityplayer).posY + 0.5, ((Entity) entityplayer).posZ + 0.5, "harrypottermod:WandFlick", 1.0f,
								Item.itemRand.nextFloat() * 0.4f + 0.8f);
						entityplayer.addPotionEffect(new PotionEffect(MainClass.ShieldEffect.id, 200, 1, false));
	...
	public void onPlayerStoppedUsing(final ItemStack par1ItemStack, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (this.getDamage(par1ItemStack) == 16 && !par2World.isRemote) {
			...
			fireball2.setPosition(((Entity) par3EntityPlayer).posX + look.xCoord * 2.0, ((Entity) par3EntityPlayer).posY + look.yCoord * 2.0, ((Entity) par3EntityPlayer).posZ + look.zCoord * 2.0);
		...
		if (this.getDamage(par1ItemStack) == 23 && !par2World.isRemote) {
			...
			fireball9.setPosition(((Entity) par3EntityPlayer).posX + look.xCoord * 2.0, ((Entity) par3EntityPlayer).posY + 1.0 + look.yCoord * 2.0,
					((Entity) par3EntityPlayer).posZ + look.zCoord * 2.0);
